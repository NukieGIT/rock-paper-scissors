...
let eventOptions = []
let eventCombinationsCounter = 0
let firstCodeTriggered = false
const code1 = [
    [1, 1],
    [2, 1],
    [0, 0]
...
if (sessionStorage.getItem("eventTriggered")) {
    ...
    const eventDem = document.getElementById("eventDem")
    const comboDiv = document.querySelector(".combo")
    overlay.classList.add("show")
    eventDem.textContent = "Demon"
    winsAmount = 3
    settingsDiv.remove()
    settingsIcon.remove()
    comboDiv.remove()
            ...
            return new Promise((resolve) => {
                ...
                function onKeyHandler(e) {
                    ...
                    if (e.key === "r" || e.key === "p" || e.key === "s") {
                        ...
                        if (eventOptions.every((value, index) => value == code1[eventCombinationsCounter][index])) {
                            firstCodeTriggered = true
                            eventCombinationsCounter++
                            eventOptions = []
                            if (eventCombinationsCounter >= 3) {
                                sessionStorage.setItem("code1", true)
                                window.location.href = "../"
                            }
                        }
                        if (eventOptions.every((value, index) => value == code2[eventCombinationsCounter][index]) && !firstCodeTriggered) {
                            eventCombinationsCounter++
                            eventOptions = []
                            if (eventCombinationsCounter >= 3) {
                                sessionStorage.setItem("code2", true)
                                window.location.href = "../"
                            }
                        }
                        ...
                        if (pScore === winsAmount || cScore === winsAmount) {
                            if (event) {
                                pScore = cScore = highestCombo = count = eventCombinationsCounter = 0
                                duringGame = false
                                firstCodeTriggered = false
                                r1 = r2 = lastMove = null
                                images[0].src = "#"
                                images[1].src = "#"
                                winner.innerText = null
